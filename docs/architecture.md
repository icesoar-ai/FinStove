...
| 类别 | 现状 | 缺失 |
|------|------|------|
| A股财报 | 年报/半年报/季报 + 三张表完整 | — |
| 美股财报 | 10-K/10-Q + yfinance 三张表 | — |
| 港股 | 日线 + 三张表 + 财务指标 + 分红 | 缺 PDF 年报 (HKEX 披露易无 API) |
| ETF | A股 (日线/净值/持仓) + 美股 (日线) | — |
| 可转债 | 不支持 | 无可转债数据 |
| 名称映射 | 指数/商品/外汇/加密硬编码中文名 | 可改为 yfinance 动态查询 (需 code→symbol 映射表，英文名) |
