...
class DDMValuation(ValuationMethod):
    ...
    def evaluate(self, financials: dict, market_data=None) -> ValuationResult:
        ...
        try:
            ...
            if dps is None or dps <= 0:
                return ValuationResult(method=self.name, fair_value=0, value_low=0, value_high=0, confidence=0.1, warnings=["公司不分红，DDM不适用"])
    ...
    def _extract_dps(self, inc, cf) -> float | None:
        # Try DPS from income statement
        for df_source in [inc, cf]:
            if df_source is None or df_source.empty:
                continue
            for col in df_source.columns:
                col_l = str(col).lower()
                if "dps" in col_l or "每股股利" in col_l or "每股分红" in col_l or "dividend" in col_l:
